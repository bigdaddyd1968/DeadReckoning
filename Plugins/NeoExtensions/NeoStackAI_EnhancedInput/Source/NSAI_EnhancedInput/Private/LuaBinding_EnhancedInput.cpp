...
static UInputAction* ResolveInputActionFromRef(const FString& ActionRef)
{
	...
	for (const FAssetData& AD : Assets)
	{
		...
		{
			UInputAction* Action = Cast<UInputAction>(AD.GetAsset());
			if (IsValid(Action))
			{
				return Action;
			}
...
static UInputMappingContext* ResolveInputMappingContextFromRef(const FString& ContextRef)
{
	if (ContextRef.IsEmpty()) return nullptr;

	if (ContextRef.Contains(TEXT("/")))
	{
		FString FullPath = ContextRef;
		if (!FullPath.Contains(TEXT(".")))
		{
		...
		if (IsValid(Context)) return Context;
	}
	...
	for (const FAssetData& AD : Assets)
	{
		if (AD.AssetName.ToString().Equals(ContextRef, ESearchCase::IgnoreCase))
		{
			UInputMappingContext* Context = Cast<UInputMappingContext>(AD.GetAsset());
			if (IsValid(Context))
			{
				return Context;
			}
	...
	if (!ContextRef.Contains(TEXT("/")))
	{
		static const TCHAR* Prefixes[] = { TEXT("/Game/Input/"), TEXT("/Game/Input/Contexts/"), TEXT("/Game/") };
		for (const TCHAR* Prefix : Prefixes)
		{
			const FString TestPath = FString(Prefix) + ContextRef + TEXT(".") + ContextRef;
			UInputMappingContext* Context = LoadObject<UInputMappingContext>(nullptr, *TestPath);
			if (IsValid(Context)) return Context;
		}
	}
...
static UWorld* GetPlaytestPIEWorld()
{
	if (!GUnrealEd)
	{
		return nullptr;
...
static sol::table MakePlaytestInputResult(sol::this_state S, bool bOk, const FString& Message)
{
...
static FString MappingQueryResultToString(EMappingQueryResult Result)
{
	if (const UEnum* Enum = StaticEnum<EMappingQueryResult>())
	{
		return Enum->GetNameStringByValue(static_cast<int64>(Result));
	}
	return FString::Printf(TEXT("%d"), static_cast<int32>(Result));
}

static FString MappingQueryIssueToString(EMappingQueryIssue Issue)
{
	if (const UEnum* Enum = StaticEnum<EMappingQueryIssue>())
	{
		return Enum->GetNameStringByValue(static_cast<int64>(Issue));
	}
	return FString::Printf(TEXT("%d"), static_cast<int32>(Issue));
}

static UEnhancedInputLocalPlayerSubsystem* GetPlaytestEnhancedInputSubsystem(FString& OutError)
{
	UWorld* World = GetPlaytestPIEWorld();
	if (!World)
	{
		OutError = TEXT("No active Play In Editor world. Call playtest_start() first.");
		return nullptr;
	}
	...
	if (!PC)
	{
		OutError = TEXT("No PIE player controller found.");
		return nullptr;
	}
	...
	if (!LocalPlayer)
	{
		OutError = TEXT("PIE player controller has no local player.");
		return nullptr;
	}
	...
	if (!Subsystem)
	{
		OutError = TEXT("Enhanced Input local player subsystem is not available for the PIE player.");
		return nullptr;
	}

	if (!Subsystem->GetPlayerInput())
	{
		OutError = TEXT("Enhanced Input player input is not available for the PIE player.");
		return nullptr;
	}
...
static FVector VectorFromValueObject(const sol::object& ValueObj)
{
	if (ValueObj.is<bool>())
	{
		return FVector(ValueObj.as<bool>() ? 1.0 : 0.0, 0.0, 0.0);
	}

	if (ValueObj.is<double>())
	{
		return FVector(ValueObj.as<double>(), 0.0, 0.0);
	}

	if (ValueObj.is<sol::table>())
	{
		sol::table T = ValueObj.as<sol::table>();
		const double X = T.get_or("x", T.get_or(1, 0.0));
		const double Y = T.get_or("y", T.get_or(2, 0.0));
		const double Z = T.get_or("z", T.get_or(3, 0.0));
...
static FInputActionValue MakeInputActionValueForAction(const UInputAction* Action, const sol::object& ValueObj)
{
	const EInputActionValueType ValueType = IsValid(Action) ? Action->ValueType : EInputActionValueType::Axis1D;
	const FVector Vec = ValueObj.valid() ? VectorFromValueObject(ValueObj) : FVector(1.0, 0.0, 0.0);
	return FInputActionValue(ValueType, Vec);
}

static FInputActionValue MakeInputActionValueFromOptions(const sol::table& Opts, const UInputAction* Action)
{
	sol::object ValueObj = Opts["value"];
	if (!ValueObj.valid() || ValueObj == sol::lua_nil)
	{
		const EInputActionValueType ValueType = IsValid(Action) ? Action->ValueType : EInputActionValueType::Boolean;
		return FInputActionValue(ValueType, FVector(1.0, 0.0, 0.0));
	}

	return MakeInputActionValueForAction(Action, ValueObj);
}

static FString GetInjectionMode(const sol::table& Opts)
{
	return NeoLuaStr::ToFString(Opts.get_or<std::string>("mode", "pulse")).ToLower();
}

static void InjectActionByMode(UEnhancedInputLocalPlayerSubsystem* Subsystem, const UInputAction* Action,
	const FInputActionValue& Value, const FString& Mode)
{
	static const TArray<UInputModifier*> NoModifiers;
	static const TArray<UInputTrigger*> NoTriggers;

	if (Mode == TEXT("start") || Mode == TEXT("hold") || Mode == TEXT("continuous"))
	{
	...
	else if (Mode == TEXT("stop") || Mode == TEXT("release"))
	{
		Subsystem->StopContinuousInputInjectionForAction(Action);
	}
	else
	{
		Subsystem->InjectInputForAction(Action, Value, NoModifiers, NoTriggers);
	}
}

static void InjectMappingByMode(UEnhancedInputLocalPlayerSubsystem* Subsystem, const FName MappingName,
	const FInputActionValue& Value, const FString& Mode)
{
	static const TArray<UInputModifier*> NoModifiers;
	static const TArray<UInputTrigger*> NoTriggers;
	...
	if (Mode == TEXT("start") || Mode == TEXT("hold") || Mode == TEXT("continuous"))
	{
		Subsystem->StartContinuousInputInjectionForPlayerMapping(MappingName, Value, NoModifiers, NoTriggers);
	}
	else if (Mode == TEXT("update"))
	{
		Subsystem->UpdateValueOfContinuousInputInjectionForPlayerMapping(MappingName, Value);
	}
	else if (Mode == TEXT("stop") || Mode == TEXT("release"))
	{
		Subsystem->StopContinuousInputInjectionForPlayerMapping(MappingName);
	}
	else
	{
		Subsystem->InjectInputForPlayerMapping(MappingName, Value, NoModifiers, NoTriggers);
	}
}
...
static NeoLuaProperty::FPropertyValueInput MakePropertyInput(const sol::object& Value)
{
	...
	const TMap<FString, FString>& Aliases, FString& OutError)
{
	...
	return NeoLuaProperty::SetNamedPropertyValue(Obj, PropName, MakePropertyInput(Value), OutError);
}
...
static UInputTrigger* CreateTriggerFromTable(UObject* Outer, const sol::table& P, FString& OutError)
{
	...
	if (!ChordRef.empty())
	{
		...
		{
			UInputAction* ChordAction = ResolveInputActionFromRef(NeoLuaStr::ToFString(ChordRef));
			if (ChordAction)
			{
		...
		if (CAProp)
		{
			...
			if (ArrProp)
			{
				...
				for (auto& Pair : CA)
				{
					...
					{
						FString Ref = NeoLuaStr::ToFString(Pair.second.as<std::string>());
						...
						if (A)
						{
							...
							FString Error;
							NeoLuaProperty::SetPropertyValueFromStringSafe(ArrProp->Inner, Arr.GetRawPtr(Idx), Trigger, ImportStr, Error);
						}
					}
				}
			}
...
static UInputModifier* CreateModifierFromTable(UObject* Outer, const sol::table& P, FString& OutError)
{
		...
		P.get<sol::optional<bool>>("negate_z").has_value())
	{
		...
		FString Error;
		if (FProperty* BX = NeoLuaProperty::FindPropertyByName(ModifierClass, TEXT("bX")))
			NeoLuaProperty::SetPropertyValue(BX, BX->ContainerPtrToValuePtr<void>(Modifier), Modifier, Input, Error);
		if (FProperty* BY = NeoLuaProperty::FindPropertyByName(ModifierClass, TEXT("bY")))
			NeoLuaProperty::SetPropertyValue(BY, BY->ContainerPtrToValuePtr<void>(Modifier), Modifier, Input, Error);
		if (FProperty* BZ = NeoLuaProperty::FindPropertyByName(ModifierClass, TEXT("bZ")))
			NeoLuaProperty::SetPropertyValue(BZ, BZ->ContainerPtrToValuePtr<void>(Modifier), Modifier, Input, Error);
	}
	...
	for (auto& Pair : P)
	{
		...
		if (PropName.Contains(TEXT(".")))
		{
			...
			if (StructProp)
			{
				...
				{
					FString PropError;
					NeoLuaProperty::SetNamedPropertyValue(StructPtr, SP->Struct, Modifier, FieldName, MakePropertyInput(Pair.second), PropError);
				}
			}
			continue;
		}
...
static TArray<FLuaFunctionDoc> EnhancedInputDocs = {
	{ TEXT("playtest_input_action(opts)"),
	  TEXT("Inject an Enhanced Input action into the active PIE local player. opts: {action='/Game/Input/IA_Move'|'IA_Move', value=true|number|{x,y,z}, mode='pulse'|'start'|'update'|'stop'}."),
	  TEXT("table") },
	{ TEXT("playtest_input_mapping(opts)"),
	  TEXT("Inject an Enhanced Input player-mappable mapping name into the active PIE local player. opts: {mapping='Move', value=true|number|{x,y,z}, mode='pulse'|'start'|'update'|'stop'}."),
	  TEXT("table") },
	{ TEXT("playtest_input_context(opts)"),
	  TEXT("Mutate/inspect active PIE Enhanced Input mapping contexts. opts: {op='add'|'remove'|'clear'|'status'|'query_keys'|'query_map_key', context='/Game/IMC', action='/Game/IA', key='SpaceBar', priority=0, force_immediately=true}."),
	  TEXT("table") }
};

static void BindEnhancedInput(sol::state& Lua, FLuaSessionData& Session)
{
	Lua.set_function("playtest_input_action", [](sol::table Opts, sol::this_state S) -> sol::table
	{
		FString Error;
		UEnhancedInputLocalPlayerSubsystem* Subsystem = GetPlaytestEnhancedInputSubsystem(Error);
		if (!Subsystem)
		{
			return MakePlaytestInputResult(S, false, Error);
		}

		const FString ActionRef = NeoLuaStr::ToFString(Opts.get_or<std::string>("action", ""));
		if (ActionRef.IsEmpty())
		{
			return MakePlaytestInputResult(S, false, TEXT("playtest_input_action requires opts.action."));
		}

		UInputAction* Action = ResolveInputActionFromRef(ActionRef);
		if (!IsValid(Action))
		{
			return MakePlaytestInputResult(S, false, FString::Printf(TEXT("InputAction not found: %s"), *ActionRef));
		}

		const FString Mode = GetInjectionMode(Opts);
		if (Mode != TEXT("pulse") && Mode != TEXT("start") && Mode != TEXT("hold") && Mode != TEXT("continuous")
			&& Mode != TEXT("update") && Mode != TEXT("stop") && Mode != TEXT("release"))
		{
			return MakePlaytestInputResult(S, false, FString::Printf(TEXT("Unsupported injection mode: %s"), *Mode));
		}

		const FInputActionValue Value = MakeInputActionValueFromOptions(Opts, Action);
		InjectActionByMode(Subsystem, Action, Value, Mode);

		sol::table Result = MakePlaytestInputResult(S, true,
			FString::Printf(TEXT("Injected action %s mode=%s value=%s"), *Action->GetName(), *Mode, *Value.ToString()));
			Result["action"] = TCHAR_TO_UTF8(*Action->GetPathName());
			Result["mode"] = TCHAR_TO_UTF8(*Mode);
			Result["value"] = TCHAR_TO_UTF8(*Value.ToString());
#if ENGINE_MAJOR_VERSION > 5 || (ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION >= 7)
			Result["has_continuous_injection"] = Subsystem->HasContinuousInputInjectionForAction(Action);
#else
			Result["has_continuous_injection"] = false;
#endif
			return Result;
		});

	Lua.set_function("playtest_input_context", [](sol::table Opts, sol::this_state S) -> sol::table
	{
		FString Error;
		UEnhancedInputLocalPlayerSubsystem* Subsystem = GetPlaytestEnhancedInputSubsystem(Error);
		if (!Subsystem)
		...
		const FString Op = NeoLuaStr::ToFString(Opts.get_or<std::string>("op", "status")).ToLower();
		FModifyContextOptions Options;
		Options.bIgnoreAllPressedKeysUntilRelease = Opts.get_or("ignore_pressed_until_release", true);
		Options.bForceImmediately = Opts.get_or("force_immediately", true);
		Options.bNotifyUserSettings = Opts.get_or("notify_user_settings", false);

		sol::table Result = MakePlaytestInputResult(S, true, FString::Printf(TEXT("Enhanced Input context op=%s"), *Op));
		...
		const FString ContextRef = NeoLuaStr::ToFString(Opts.get_or<std::string>("context", ""));
		UInputMappingContext* Context = nullptr;
		if (!ContextRef.IsEmpty())
		{
			Context = ResolveInputMappingContextFromRef(ContextRef);
			if (!IsValid(Context))
			{
				return MakePlaytestInputResult(S, false, FString::Printf(TEXT("InputMappingContext not found: %s"), *ContextRef));
			}
			Result["context"] = TCHAR_TO_UTF8(*Context->GetPathName());
		}
		...
		if (Op == TEXT("add"))
		{
			if (!IsValid(Context))
			{
				return MakePlaytestInputResult(S, false, TEXT("playtest_input_context add requires opts.context."));
			}
			const int32 Priority = Opts.get_or("priority", 0);
			Subsystem->AddMappingContext(Context, Priority, Options);
			int32 FoundPriority = INDEX_NONE;
			const bool bHasContext = Subsystem->HasMappingContext(Context, FoundPriority);
			Result["has_context"] = bHasContext;
			Result["priority"] = FoundPriority;
			return Result;
		}
		...
		if (Op == TEXT("remove"))
		{
			if (!IsValid(Context))
			{
				return MakePlaytestInputResult(S, false, TEXT("playtest_input_context remove requires opts.context."));
			}
		...
		if (!ActionRef.IsEmpty())
		{
			Action = ResolveInputActionFromRef(ActionRef);
			if (!IsValid(Action))
			{
				return MakePlaytestInputResult(S, false, FString::Printf(TEXT("InputAction not found: %s"), *ActionRef));
			}
		...
		if (Op == TEXT("query_keys"))
		{
			if (!IsValid(Action))
			{
				return MakePlaytestInputResult(S, false, TEXT("playtest_input_context query_keys requires opts.action."));
			}
			const TArray<FKey> Keys = Subsystem->QueryKeysMappedToAction(Action);
			sol::state_view LuaView(S);
			sol::table KeyRows = LuaView.create_table();
			int32 Row = 1;
			for (const FKey& Key : Keys)
			{
				KeyRows[Row++] = TCHAR_TO_UTF8(*Key.GetFName().ToString());
			}
			Result["keys"] = KeyRows;
			Result["key_count"] = Keys.Num();
			return Result;
		}

		if (Op == TEXT("query_map_key"))
		{
			if (!IsValid(Context))
			{
				return MakePlaytestInputResult(S, false, TEXT("playtest_input_context query_map_key requires opts.context."));
			}
			if (!IsValid(Action))
			{
				return MakePlaytestInputResult(S, false, TEXT("playtest_input_context query_map_key requires opts.action."));
			}
			const FString KeyName = NeoLuaStr::ToFString(Opts.get_or<std::string>("key", ""));
			const FKey Key = ResolveKey(KeyName);
			if (!Key.IsValid())
			{
				return MakePlaytestInputResult(S, false, FString::Printf(TEXT("Invalid key: %s"), *KeyName));
			}

			TArray<FMappingQueryIssue> Issues;
			const EMappingQueryResult QueryResult = Subsystem->QueryMapKeyInActiveContextSet(
				Context, Action, Key, Issues, DefaultMappingIssues::StandardFatal);
			Result["query_result"] = TCHAR_TO_UTF8(*MappingQueryResultToString(QueryResult));
			Result["issue_count"] = Issues.Num();
			sol::state_view LuaView(S);
			sol::table IssueRows = LuaView.create_table();
			for (int32 Index = 0; Index < Issues.Num(); ++Index)
			{
				sol::table Row = LuaView.create_table();
				Row["issue"] = TCHAR_TO_UTF8(*MappingQueryIssueToString(Issues[Index].Issue));
				Row["blocking_context"] = Issues[Index].BlockingContext
					? TCHAR_TO_UTF8(*Issues[Index].BlockingContext->GetPathName())
					: "";
				Row["blocking_action"] = Issues[Index].BlockingAction
					? TCHAR_TO_UTF8(*Issues[Index].BlockingAction->GetPathName())
					: "";
				IssueRows[Index + 1] = Row;
			}
			Result["issues"] = IssueRows;
			return Result;
		}

		return MakePlaytestInputResult(S, false, FString::Printf(TEXT("Unsupported input context op: %s"), *Op));
	});

	Lua.set_function("playtest_input_mapping", [](sol::table Opts, sol::this_state S) -> sol::table
	{
		FString Error;
		UEnhancedInputLocalPlayerSubsystem* Subsystem = GetPlaytestEnhancedInputSubsystem(Error);
		if (!Subsystem)
		...
		const FString Mapping = NeoLuaStr::ToFString(Opts.get_or<std::string>("mapping", ""));
		if (Mapping.IsEmpty())
		{
			return MakePlaytestInputResult(S, false, TEXT("playtest_input_mapping requires opts.mapping."));
		}
		...
		UInputAction* TypeHintAction = nullptr;
		const FString ActionRef = NeoLuaStr::ToFString(Opts.get_or<std::string>("action", ""));
		if (!ActionRef.IsEmpty())
		{
			TypeHintAction = ResolveInputActionFromRef(ActionRef);
		}

		const FInputActionValue Value = MakeInputActionValueFromOptions(Opts, TypeHintAction);
		InjectMappingByMode(Subsystem, FName(*Mapping), Value, Mode);

		sol::table Result = MakePlaytestInputResult(S, true,
			FString::Printf(TEXT("Injected mapping %s mode=%s value=%s"), *Mapping, *Mode, *Value.ToString()));
		Result["mapping"] = TCHAR_TO_UTF8(*Mapping);
		Result["mode"] = TCHAR_TO_UTF8(*Mode);
		Result["value"] = TCHAR_TO_UTF8(*Value.ToString());
		return Result;
	});
	...
	Lua.set_function("_enrich_input_action", [&Session](sol::table AssetObj, sol::this_state S)
	{
		...
		UInputAction* Action = NeoLuaAsset::Resolve<UInputAction>(FPath);
		if (!Action) return;
			...
			const std::string& Type, sol::optional<sol::table> Params, sol::this_state S) -> sol::object
		{
			...
			if (FType.Equals(TEXT("trigger"), ESearchCase::IgnoreCase))
			{
				...
				const FScopedTransaction Txn(NSLOCTEXT("NeoStackAI", "LuaAddTrigger", "Lua: Add Trigger"));
				Action->Modify();
			...
			else if (FType.Equals(TEXT("modifier"), ESearchCase::IgnoreCase))
			{
				...
				const FScopedTransaction Txn(NSLOCTEXT("NeoStackAI", "LuaAddModifier", "Lua: Add Modifier"));
				Action->Modify();
			...
			const std::string& Type, sol::object Id, sol::this_state S) -> sol::object
		{
			...
			if (FType.Equals(TEXT("trigger"), ESearchCase::IgnoreCase))
			{
				...
				const FScopedTransaction Txn(NSLOCTEXT("NeoStackAI", "LuaRemoveTrigger", "Lua: Remove Trigger"));
				Action->Modify();
			...
			else if (FType.Equals(TEXT("modifier"), ESearchCase::IgnoreCase))
			{
				...
				const FScopedTransaction Txn(NSLOCTEXT("NeoStackAI", "LuaRemoveModifier", "Lua: Remove Modifier"));
				Action->Modify();
			...
			sol::table Params, sol::this_state S) -> sol::object
		{
			...
			const FScopedTransaction Txn(NSLOCTEXT("NeoStackAI", "LuaConfigureAction", "Lua: Configure InputAction"));
			Action->Modify();
			...
			if (!VTStr.empty())
			{
				FString VT = NeoLuaStr::ToFString(VTStr);
			...
			if (!AccStr.empty())
			{
				FString Acc = NeoLuaStr::ToFString(AccStr);
			...
			if (!DescStr.empty())
			{
				Action->ActionDescription = FText::FromString(NeoLuaStr::ToFString(DescStr));
				Changes++;
			}
	...
	Lua.set_function("_enrich_mapping_context", [&Session](sol::table AssetObj, sol::this_state S)
	{
		...
		std::string PathStr = AssetObj.get<std::string>("path");
		FString FPath = NeoLuaStr::ToFString(PathStr);
		UInputMappingContext* Context = NeoLuaAsset::Resolve<UInputMappingContext>(FPath);
		if (!Context) return;
			...
			const std::string& Type, sol::optional<sol::table> Params, sol::this_state S) -> sol::object
		{
			...
			if (FType.Equals(TEXT("mapping"), ESearchCase::IgnoreCase))
			{
				...
				UInputAction* Action = ResolveInputActionFromRef(NeoLuaStr::ToFString(ActionRef));
				if (!Action)
				{
				...
				FKey Key = ResolveKey(NeoLuaStr::ToFString(KeyStr));
				if (!Key.IsValid())
				{
				...
				const FScopedTransaction Txn(NSLOCTEXT("NeoStackAI", "LuaAddMapping", "Lua: Add Mapping"));
				Context->Modify();
		...
		else if (FType.Equals(TEXT("profile_override"), ESearchCase::IgnoreCase))
			{
				...
				const FScopedTransaction Txn(NSLOCTEXT("NeoStackAI", "LuaAddProfileOverride", "Lua: Add Profile Override Mapping"));
				Context->Modify();

				FString FProfileId = NeoLuaStr::ToFString(ProfileId);
			...
			const std::string& Type, sol::object Id, sol::this_state S) -> sol::object
		{
			...
			if (FType.Equals(TEXT("mapping"), ESearchCase::IgnoreCase))
			{
				...
				if (Id.is<int>())
				{
					...
					const FScopedTransaction Txn(NSLOCTEXT("NeoStackAI", "LuaRemoveMapping", "Lua: Remove Mapping"));
					Context->Modify();
				...
				if (Id.is<sol::table>())
				{
					...
					const FScopedTransaction Txn(NSLOCTEXT("NeoStackAI", "LuaRemoveMappingByKey", "Lua: Remove Mapping by Key"));
					Context->Modify();
			...
			else if (FType.Equals(TEXT("all_mappings"), ESearchCase::IgnoreCase))
			{
				...
				const FScopedTransaction Txn(NSLOCTEXT("NeoStackAI", "LuaUnmapAll", "Lua: Unmap All"));
				Context->Modify();
				Context->UnmapAll();
				Context->GetPackage()->MarkPackageDirty();
			...
			else if (FType.Equals(TEXT("all_for_action"), ESearchCase::IgnoreCase))
			{
				...
				FString ActionRef = NeoLuaStr::ToFString(Id.as<std::string>());
				UInputAction* Action = ResolveInputActionFromRef(ActionRef);
				if (!Action)
				{
				...
				const FScopedTransaction Txn(NSLOCTEXT("NeoStackAI", "LuaUnmapAllForAction", "Lua: Unmap All Keys From Action"));
				Context->Modify();
		...
		else if (FType.Equals(TEXT("profile_override"), ESearchCase::IgnoreCase))
			{
				...
				const FScopedTransaction Txn(NSLOCTEXT("NeoStackAI", "LuaRemoveProfileOverride", "Lua: Remove Profile Override"));
				Context->Modify();
			...
			const std::string& Type, sol::object IndexObj, sol::optional<sol::table> Params, sol::this_state S) -> sol::object
		{
			...
			const FScopedTransaction Txn(NSLOCTEXT("NeoStackAI", "LuaConfigureMapping", "Lua: Configure Mapping"));
			Context->Modify();
			...
			if (!NewKeyStr.empty())
			{
				FKey NewKey = ResolveKey(NeoLuaStr::ToFString(NewKeyStr));
				if (NewKey.IsValid())
				{
					Mapping.Key = NewKey;
					Changes++;
		...
		if (FType.Equals(TEXT("profiles"), ESearchCase::IgnoreCase))
			...
			if (FType.Equals(TEXT("profile_override"), ESearchCase::IgnoreCase) || FType.Equals(TEXT("profile_overrides"), ESearchCase::IgnoreCase))
			{
				...
				FString FProfileId = NeoLuaStr::ToFString(ArgOpt.value());
				TMap<FString, FInputMappingContextMappingData>* Overrides = GetMappingProfileOverrides(Context);
				if (!Overrides)
				{
