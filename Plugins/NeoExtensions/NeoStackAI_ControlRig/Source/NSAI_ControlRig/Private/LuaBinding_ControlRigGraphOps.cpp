...
{
	return NeoLuaEnum::ParseOr<ERigVMPinDirection>(Dir, ERigVMPinDirection::Input);
}

static sol::table BuildNodeResultTable(sol::state_view LuaView, FLuaSessionData& Session, UEdGraphNode* Node)
{
...
static void BindControlRigGraphOps(sol::state& Lua, FLuaSessionData& Session)
{
	...
	Lua.set_function("_cr_add_node", [&Session](sol::lightuserdata_value GraphUD,
		sol::object NodeArg, double X, double Y, sol::this_state S) -> sol::object
	{
		...
		{
			Session.Log(TEXT("[FAIL] _cr_add_node -> 'node' must be a string name or a result table from find_nodes()"));
			return sol::lua_nil;
		}
		...
		return BuildNodeResultTable(LuaView, Session, Node);
	});

	Lua.set_function("_cr_find_nodes", [&Session](sol::lightuserdata_value GraphUD, const std::string& Query,
		int32 MaxResults, sol::this_state S) -> sol::object
	{
		sol::state_view LuaView(S);
		UEdGraph* Graph = reinterpret_cast<UEdGraph*>(GraphUD.value);
		if (!Graph || !LuaControlRig::IsRigVMGraph(Graph))
		{
			Session.Log(TEXT("[FAIL] _cr_find_nodes -> invalid or non-RigVM graph"));
			return LuaView.create_table();
		}

		TArray<LuaControlRig::FRigVMNodeSearchResult> CRResults =
			LuaControlRig::FindNodes(NeoLuaStr::ToFString(Query), MaxResults);

		sol::table ResultTable = LuaView.create_table();
		for (int32 Index = 0; Index < CRResults.Num(); ++Index)
		{
			const LuaControlRig::FRigVMNodeSearchResult& CR = CRResults[Index];
			sol::table Entry = LuaView.create_table();
			Entry["name"] = TCHAR_TO_UTF8(*CR.Name);
			Entry["category"] = TCHAR_TO_UTF8(*CR.Category);
			Entry["tooltip"] = TCHAR_TO_UTF8(*CR.Keywords);
			Entry["score"] = CR.Score;
			ResultTable[Index + 1] = Entry;
		}

		Session.Log(FString::Printf(TEXT("[OK] find_nodes(\"%s\") -> %d results"), UTF8_TO_TCHAR(Query.c_str()), CRResults.Num()));
		return ResultTable;
	});

	Lua.set_function("_cr_connect", [&Session](const std::string& FromHandle, const std::string& FromPinName,
		const std::string& ToHandle, const std::string& ToPinName, sol::this_state S) -> sol::object
	{
		...
		if (!FromNode) { Session.Log(FString::Printf(TEXT("[FAIL] connect -> source node \"%s\" not found"), *FFromHandle)); return sol::lua_nil; }
		if (!ToNode) { Session.Log(FString::Printf(TEXT("[FAIL] connect -> target node \"%s\" not found"), *FToHandle)); return sol::lua_nil; }

		FString Error;
		if (LuaControlRig::Connect(FromNode->GetGraph(), FromNode, FFromPin, ToNode, FToPin, Error))
		{
			Session.Log(FString::Printf(TEXT("[OK] connect(%s:%s -> %s:%s)"), *FFromHandle, *FFromPin, *FToHandle, *FToPin));
	...
	Lua.set_function("_cr_disconnect", [&Session](const std::string& Handle, const std::string& PinName, sol::this_state S) -> sol::object
	{
		...
		if (!Node) { Session.Log(FString::Printf(TEXT("[FAIL] disconnect -> node \"%s\" not found"), *FHandle)); return sol::lua_nil; }

		FString Error;
		if (LuaControlRig::Disconnect(Node->GetGraph(), Node, FPinName, Error))
		{
	...
	Lua.set_function("_cr_disconnect_all", [&Session](const std::string& Handle, sol::this_state S) -> sol::object
	{
		FString FHandle = NeoLuaStr::ToFString(Handle);
		UEdGraphNode* Node = Session.FindNode(FHandle);
		if (!Node) { Session.Log(FString::Printf(TEXT("[FAIL] disconnect_all -> node \"%s\" not found"), *FHandle)); return sol::lua_nil; }

		URigVMController* Controller = LuaControlRig::GetController(Node->GetGraph());
		if (!Controller) { Session.Log(TEXT("[FAIL] disconnect_all -> no RigVM controller")); return sol::lua_nil; }

		URigVMNode* ModelNode = LuaControlRig::GetModelNode(Node);
		if (!ModelNode) { Session.Log(TEXT("[FAIL] disconnect_all -> could not find model node")); return sol::lua_nil; }

		Controller->OpenUndoBracket(TEXT("Disconnect All"));
		for (URigVMPin* Pin : ModelNode->GetAllPinsRecursively())
		{
			if (!Pin)
			{
				continue;
	...
	Lua.set_function("_cr_disconnect_from", [&Session](const std::string& FromHandle, const std::string& FromPinName,
		const std::string& ToHandle, const std::string& ToPinName, sol::this_state S) -> sol::object
	{
		FString FFromHandle = NeoLuaStr::ToFString(FromHandle);
		FString FToHandle = NeoLuaStr::ToFString(ToHandle);
		...
		if (!Controller) { Session.Log(TEXT("[FAIL] disconnect_from -> no RigVM controller")); return sol::lua_nil; }
		...
		if (OutputPath.IsEmpty() || InputPath.IsEmpty()) { Session.Log(TEXT("[FAIL] disconnect_from -> could not resolve pin paths")); return sol::lua_nil; }
	...
	Lua.set_function("_cr_split_pin", [&Session](const std::string& Handle, const std::string& PinName, sol::this_state S) -> sol::object
	{
		FString FHandle = NeoLuaStr::ToFString(Handle);
		FString FPinName = NeoLuaStr::ToFString(PinName);
		UEdGraphNode* Node = Session.FindNode(FHandle);
		...
		URigVMController* Controller = LuaControlRig::GetController(Node->GetGraph());
		if (!Controller) { Session.Log(TEXT("[FAIL] split_pin -> no RigVM controller")); return sol::lua_nil; }
	...
	Lua.set_function("_cr_recombine_pin", [&Session](const std::string& Handle, const std::string& PinName, sol::this_state S) -> sol::object
	{
		FString FHandle = NeoLuaStr::ToFString(Handle);
		FString FPinName = NeoLuaStr::ToFString(PinName);
		UEdGraphNode* Node = Session.FindNode(FHandle);
		...
		URigVMController* Controller = LuaControlRig::GetController(Node->GetGraph());
		if (!Controller) { Session.Log(TEXT("[FAIL] recombine_pin -> no RigVM controller")); return sol::lua_nil; }
	...
	Lua.set_function("_cr_set_node_comment", [&Session](const std::string& Handle, const std::string& Text, bool Visible, sol::this_state S) -> sol::object
	{
		FString FHandle = NeoLuaStr::ToFString(Handle);
		...
		URigVMController* Controller = LuaControlRig::GetController(Node->GetGraph());
		if (!Controller) { Session.Log(TEXT("[FAIL] set_node_comment -> no RigVM controller")); return sol::lua_nil; }

		FName NodeName = LuaControlRig::GetModelNodeName(Node);
		if (Controller->SetCommentTextByName(NodeName, FText, 18, Visible, false, false, false))
		{
			Session.Log(FString::Printf(TEXT("[OK] set_node_comment(%s) via RigVM"), *FHandle));
			return sol::make_object(S, true);
		}

		Session.Log(TEXT("[FAIL] set_node_comment -> SetCommentTextByName failed"));
		return sol::lua_nil;
	});

	Lua.set_function("_cr_delete_node", [&Session](const std::string& Handle, sol::this_state S) -> sol::object
	{
		FString FHandle = NeoLuaStr::ToFString(Handle);
		UEdGraphNode* Node = Session.FindNode(FHandle);
		if (!Node) { Session.Log(FString::Printf(TEXT("[FAIL] delete_nodes -> node \"%s\" not found"), *FHandle)); return sol::lua_nil; }
		...
		sol::optional<std::string> DefaultValue, sol::this_state S) -> sol::object
	{
		...
		FString PinPath = LuaControlRig::BuildPinPath(Node, NeoLuaStr::ToFString(PinName));
		FString DefVal = NeoLuaStr::ToFStringOpt(DefaultValue);
		...
		const std::string& Variable, sol::this_state S) -> sol::object
	{
		...
		FString PinPath = LuaControlRig::BuildPinPath(Node, NeoLuaStr::ToFString(PinName));
		FString VarPath = NeoLuaStr::ToFString(Variable);
	...
	auto GetCtrlFromContext = [&Session](sol::optional<std::string> GraphHandle, const FString& FuncName) -> URigVMController*
	{
		if (GraphHandle)
		{
			FString FHandle = NeoLuaStr::ToFString(GraphHandle.value());
		...
		sol::optional<std::string> DefaultValue, sol::optional<std::string> GraphHandle, sol::this_state S) -> sol::object
	{
		...
		{
			FString DV = NeoLuaStr::ToFString(DefaultValue.value());
		...
		{
			EffectiveDefVal = NeoLuaStr::ToFString(DefaultValue.value());
		}
		...
		ERigVMPinDirection Dir = ParseRigVMDirection(NeoLuaStr::ToFString(Direction));
		FString FCppType = NeoLuaStr::ToFString(CppType);
	...
	Lua.set_function("cr_set_node_category", [&Session](const std::string& Handle, const std::string& Category, sol::this_state S) -> sol::object
	{
		...
		FName NodeName = LuaControlRig::GetModelNodeName(Node);
		if (!Ctrl->SetNodeCategoryByName(NodeName, NeoLuaStr::ToFString(Category), false, false))
		{
	...
	Lua.set_function("cr_set_node_keywords", [&Session](const std::string& Handle, const std::string& Keywords, sol::this_state S) -> sol::object
	{
		...
		FName NodeName = LuaControlRig::GetModelNodeName(Node);
		if (!Ctrl->SetNodeKeywordsByName(NodeName, NeoLuaStr::ToFString(Keywords), false, false))
		{
	...
	Lua.set_function("cr_set_node_description", [&Session](const std::string& Handle, const std::string& Description, sol::this_state S) -> sol::object
	{
		...
		FName NodeName = LuaControlRig::GetModelNodeName(Node);
		if (!Ctrl->SetNodeDescriptionByName(NodeName, NeoLuaStr::ToFString(Description), false, false))
		{
		...
		sol::optional<std::string> Category, sol::this_state S) -> sol::object
	{
		...
		{
			bSuccess = Ctrl->SetPinCategory(PinPath, NeoLuaStr::ToFString(Category.value()), false, false);
		}
		...
		sol::optional<std::string> GraphHandle, sol::this_state S) -> sol::object
	{
		...
		{
			FString FObjPath = NeoLuaStr::ToFString(TypeObjectPath.value());
		...
		sol::optional<std::string> GraphHandle, sol::this_state S) -> sol::object
	{
		...
		FName VarName = FName(UTF8_TO_TCHAR(Name.c_str()));
		FString DefVal = NeoLuaStr::ToFString(DefaultValue);
	...
	Lua.set_function("cr_collapse_nodes", [&Session](sol::table Handles, sol::optional<std::string> NameOpt, sol::this_state S) -> sol::object
	{
		...
		for (auto& Pair : Handles)
		{
			...
			FString FHandle = NeoLuaStr::ToFString(HandleOpt.value());
			UEdGraphNode* Node = Session.FindNode(FHandle);
		...
		FString CollapseName = NeoLuaStr::ToFStringOpt(NameOpt);
		...
		sol::optional<std::string> FunctionPath, sol::this_state S) -> sol::object
	{
		...
		UEdGraph* SourceGraph = Node->GetGraph();
		FString ExistingPath = NeoLuaStr::ToFStringOpt(FunctionPath);
