...
{
	FString AssetPath = NeoLuaStr::ToFString(PathStr);
	if (!AssetPath.StartsWith(TEXT("/")))
	{
		AssetPath = TEXT("/Game/") + AssetPath;
	}

	UGeometryCollection* GC = NeoLuaAsset::Resolve<UGeometryCollection>(AssetPath);
	if (!GC)
	{
...
static void PostFractureFinalize(UGeometryCollection* GC, FGeometryCollection* Collection, int32 FirstNewTransformIndex)
{
	...
	if (FirstNewTransformIndex > INDEX_NONE)
	{
		GeometryCollection::GenerateTemporaryGuids(Collection, FirstNewTransformIndex, false);
	}
...
static void BindChaosFracture(sol::state& Lua, FLuaSessionData& Session)
{
	...
	Lua.set_function("fracture_voronoi", [&Session](const std::string& AssetPathStr, sol::optional<sol::table> ParamsOpt, sol::this_state S) -> sol::object
	{
		...
		int32 InitialTransformCount = Collection->NumElements(FGeometryCollection::TransformGroup);
		...
		PostFractureFinalize(GC, Collection, InitialTransformCount);
	...
	Lua.set_function("fracture_cluster", [&Session](const std::string& AssetPathStr, sol::optional<sol::table> ParamsOpt, sol::this_state S) -> sol::object
	{
		...
		if (ClusterIndex < 0 || ClusterIndex >= NumTransforms)
		{
			Session.Log(FString::Printf(TEXT("[FAIL] fracture_cluster -> cluster_index %d out of range (0-%d)"),
				ClusterIndex, NumTransforms - 1));
			return sol::lua_nil;
		}
