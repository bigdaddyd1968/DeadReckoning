...
static void BindChaosOutfit(sol::state& Lua, FLuaSessionData& Session)
{
		...
		sol::object SizeId, sol::this_state S) -> sol::object
	{
		...
		{
			FString SizeName = NeoLuaStr::ToFString(SizeId.as<std::string>());
		...
		sol::optional<std::string> GuidOpt, sol::optional<int> BodySizeOpt, sol::this_state S) -> sol::object
	{
		...
		{
			const FString GuidString = NeoLuaStr::ToFStringOpt(GuidOpt);
			FGuid ParsedGuid;
			if (!FGuid::Parse(GuidString, ParsedGuid))
			{
				Session.Log(TEXT("[FAIL] outfit_get_pieces -> malformed outfit GUID: ") + GuidString);
				return sol::lua_nil;
			}
			if (!Guids.Contains(ParsedGuid))
			{
				Session.Log(TEXT("[FAIL] outfit_get_pieces -> unknown outfit GUID: ") + ParsedGuid.ToString());
				return sol::lua_nil;
			}
		...
		int32 BodySize = 0;
		if (BodySizeOpt.has_value())
		{
			if (BodySizeOpt.value() < 1)
			{
				Session.Log(TEXT("[FAIL] outfit_get_pieces -> body_size must be 1-based"));
				return sol::lua_nil;
			}
			BodySize = BodySizeOpt.value() - 1; // Lua 1-based to engine 0-based
		}

		TArray<int32> BodySizes = Facade.GetOutfitBodySizes(TargetGuid);
		if (!BodySizes.Contains(BodySize))
		{
			Session.Log(TEXT("[FAIL] outfit_get_pieces -> body_size is not valid for outfit GUID: ") + TargetGuid.ToString());
			return sol::lua_nil;
		}
	...
	Lua.set_function("outfit_list_guids", [&Session](const std::string& AssetPathStr, sol::this_state S) -> sol::object
	{
		...
		for (int32 i = 0; i < Guids.Num(); ++i)
		{
			...
			{
				SizesTable[j + 1] = BodySizes[j] + 1; // engine 0-based to Lua 1-based
			}
