...
static int32 ApplyLuaPropertiesToSettingsCopy(
	const UScriptStruct* SettingsType,
	const FIKRigSettingsBase* SourceSettings,
	sol::table Params,
	UObject* Owner,
	FStructOnScope& SettingsCopy)
{
	if (!SettingsType || !SourceSettings || !SettingsType->IsChildOf(FIKRigSettingsBase::StaticStruct()))
	{
		return INDEX_NONE;
	}

	SettingsCopy = FStructOnScope(SettingsType);
	if (!SettingsCopy.IsValid())
	{
		return INDEX_NONE;
	}

	SettingsType->CopyScriptStruct(SettingsCopy.GetStructMemory(), SourceSettings);

	int32 Applied = 0;
	for (auto& [key, val] : Params)
	{
		if (!key.is<std::string>()) continue;
		FString PropName = NeoLuaStr::ToFString(key.as<std::string>());
		...
		FString PropError;
		if (NeoLuaProperty::SetPropertyValue(Prop, Prop->ContainerPtrToValuePtr<void>(SettingsCopy.GetStructMemory()), Owner, MakePropertyInput(val), PropError))
		{
			Applied++;
		}
	}
...
static void BindIKRig(sol::state& Lua, FLuaSessionData& Session)
{
	Lua.set_function("_enrich_ikrig", [&Session](sol::table AssetObj, sol::this_state S)
	{
		...
		UIKRigDefinition* IKRig = NeoLuaAsset::Resolve<UIKRigDefinition>(FPath);
		if (!IKRig) return;
			...
			const std::string& Type, sol::optional<sol::table> Params, sol::this_state S) -> sol::object
		{
			...
			if (FType.Equals(TEXT("solver"), ESearchCase::IgnoreCase))
			{
				...
				FString SolverPath = ResolveIKSolverPath(NeoLuaStr::ToFString(SolverType));
				...
				if (!Root.empty()) { FString RootStr = NeoLuaStr::ToFString(Root); Ctrl->SetStartBone(FName(RootStr), Idx); }
				if (!End.empty()) { FString EndStr = NeoLuaStr::ToFString(End); Ctrl->SetEndBone(FName(EndStr), Idx); }
			...
			else if (FType.Equals(TEXT("goal"), ESearchCase::IgnoreCase))
			{
				...
				FString GoalStr = NeoLuaStr::ToFString(GoalName);
			...
			else if (FType.Equals(TEXT("chain"), ESearchCase::IgnoreCase))
			{
				...
				FString ChainStr = NeoLuaStr::ToFString(ChainName);
				FString StartStr = NeoLuaStr::ToFString(StartBone);
				FString EndStr = NeoLuaStr::ToFString(EndBone);
				FName Goal = NAME_None;
				std::string GoalStr = P.get_or<std::string>("goal", "");
				if (!GoalStr.empty()) { FString G = NeoLuaStr::ToFString(GoalStr); Goal = FName(G); }
			...
			const std::string& Type, sol::object Id, sol::this_state S) -> sol::object
		{
			...
			else if (FType.Equals(TEXT("goal"), ESearchCase::IgnoreCase))
			{
				...
				FString GoalStr = NeoLuaStr::ToFString(Id.as<std::string>());
			...
			else if (FType.Equals(TEXT("chain"), ESearchCase::IgnoreCase))
			{
				...
				FString ChainStr = NeoLuaStr::ToFString(Id.as<std::string>());
			...
			const std::string& Type, sol::object Id, sol::table Params, sol::this_state S) -> sol::object
		{
			...
			else if (FType.Equals(TEXT("goal"), ESearchCase::IgnoreCase))
			{
				...
				if (!NewBone.empty())
				{
					FString BoneStr = NeoLuaStr::ToFString(NewBone);
				...
				if (PosAlpha.has_value() || RotAlpha.has_value())
				{
					UIKRigEffectorGoal* GoalObj = Ctrl->GetGoal(GoalName);
					if (GoalObj)
					{
						FScopedReinitializeIKRig ReinitializeGoalAlpha(Ctrl, true /*bGoalsChanged*/);
						Ctrl->GetAsset()->Modify();
						Ctrl->ModifyGoal(GoalName);
				...
				if (!NewName.empty())
				{
					FString NewNameStr = NeoLuaStr::ToFString(NewName);
			...
			else if (FType.Equals(TEXT("chain"), ESearchCase::IgnoreCase))
			{
				...
				if (!StartBone.empty())
				{
					FString BoneStr = NeoLuaStr::ToFString(StartBone);
				...
				if (!EndBone.empty())
				{
					FString BoneStr = NeoLuaStr::ToFString(EndBone);
				...
				if (!Goal.empty())
				{
					FString GoalStr = NeoLuaStr::ToFString(Goal);
			...
			sol::optional<std::string> TypeOpt, sol::this_state S) -> sol::object
		{
			...
			if (FType.Equals(TEXT("solvers"), ESearchCase::IgnoreCase) || FType.Equals(TEXT("solver"), ESearchCase::IgnoreCase))
			{
				...
				for (int32 i = 0; i < Count; i++)
				{
					...
					E["index"] = i;
					const FString UniqueName = Ctrl->GetSolverUniqueName(i);
					E["label"] = TCHAR_TO_UTF8(*UniqueName);
					E["unique_name"] = TCHAR_TO_UTF8(*UniqueName);
					E["type"] = "";
					E["type_path"] = "";
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION >= 6
					if (FInstancedStruct* SolverStruct = Ctrl->GetSolverStructAtIndex(i))
					{
						if (const UScriptStruct* ScriptStruct = SolverStruct->GetScriptStruct())
						{
							E["type"] = TCHAR_TO_UTF8(*ScriptStruct->GetName());
							E["type_path"] = TCHAR_TO_UTF8(*ScriptStruct->GetPathName());
						}
					}
#endif
			...
			int SolverIndex, sol::table Params, sol::this_state S) -> sol::object
		{
			...
			FStructOnScope SettingsCopy;
			const int32 Applied = ApplyLuaPropertiesToSettingsCopy(SettingsType, Settings, Params, Ctrl->GetAsset(), SettingsCopy);
			if (Applied == INDEX_NONE)
			{
				Session.Log(FString::Printf(TEXT("[FAIL] configure_solver -> solver %d settings copy failed"), SolverIndex));
				return sol::lua_nil;
			}

			if (Applied > 0)
			{
				Solver->SetSolverSettings(reinterpret_cast<FIKRigSolverSettingsBase*>(SettingsCopy.GetStructMemory()));
			...
			sol::table Bones, sol::this_state S) -> sol::object
		{
			...
			for (auto& [_, val] : Bones)
			{
				...
				FString BoneStr = NeoLuaStr::ToFString(val.as<std::string>());
			...
			const std::string& BoneName, int SolverIndex, sol::table Params, sol::this_state S) -> sol::object
		{
			...
			FStructOnScope SettingsCopy;
			const int32 Applied = ApplyLuaPropertiesToSettingsCopy(SettingsType, BoneSettings, Params, Ctrl->GetAsset(), SettingsCopy);
			if (Applied == INDEX_NONE)
			{
				Session.Log(FString::Printf(TEXT("[FAIL] set_bone_settings -> solver %d settings copy failed"), SolverIndex));
				return sol::lua_nil;
			}

			if (Applied > 0)
			{
				Solver->SetBoneSettings(FName(BoneStr), reinterpret_cast<FIKRigBoneSettingsBase*>(SettingsCopy.GetStructMemory()));
				Ctrl->BroadcastNeedsReinitialized();
				Ctrl->GetAsset()->MarkPackageDirty();
			}
