...
#define NSAI_HAS_METAHUMAN_IDENTITY 1
#else
#define NSAI_HAS_METAHUMAN_IDENTITY 0
#endif
#endif
...
#define NSAI_HAS_METAHUMAN_WARDROBE 1
#else
#define NSAI_HAS_METAHUMAN_WARDROBE 0
#endif
...
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION >= 7
static bool ParseMHClothingVisibilityState(const FString& StateStr, EMetaHumanClothingVisibilityState& OutState, FString& OutError)
{
	if (StateStr.Equals(TEXT("Shown"), ESearchCase::IgnoreCase))
	{
		OutState = EMetaHumanClothingVisibilityState::Shown;
		return true;
	}
	if (StateStr.Equals(TEXT("Hidden"), ESearchCase::IgnoreCase))
	{
		OutState = EMetaHumanClothingVisibilityState::Hidden;
		return true;
	}
	if (StateStr.Equals(TEXT("UseOverrideMaterial"), ESearchCase::IgnoreCase) ||
		StateStr.Equals(TEXT("OverrideMaterial"), ESearchCase::IgnoreCase))
	{
		OutState = EMetaHumanClothingVisibilityState::UseOverrideMaterial;
		return true;
	}

	OutError = FString::Printf(TEXT("unknown clothing visibility state '%s' (expected Shown, Hidden, or UseOverrideMaterial)"), *StateStr);
	return false;
}
...
{
	// Routes through the shared resolver: prepends /Game/ for unrooted paths and
	// falls back to Asset Registry lookup like open_asset, so MetaHuman wrappers
	// accept the same path shapes as the rest of the Lua surface.
	FString Path = NeoLuaStr::ToFString(AssetPath);
	UMetaHumanCharacter* MHC = NeoLuaAsset::ResolveWithRegistry<UMetaHumanCharacter>(Path);
	if (!MHC)
	{
...
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION >= 7
static FMetaHumanCharacterEditorBuildParameters MakeMHBuildParamsFromAssemblySettings(const UMetaHumanCharacter* MHC)
{
	...
	BuildParams.PipelineType = AS.PipelineType;
	BuildParams.PipelineQuality = AS.PipelineQuality;
	BuildParams.AnimationSystemName = AS.AnimationSystemName;
	BuildParams.AbsoluteBuildPath = AS.RootDirectory.Path;
	BuildParams.CommonFolderPath = AS.CommonDirectory.Path;
	BuildParams.NameOverride = AS.NameOverride;
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION < 8
	BuildParams.bBakeMakeup = AS.bBakeMakeup;
	BuildParams.bExportZipFile = AS.bExportZipFile;
#endif
	return BuildParams;
}
#endif
...
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION >= 8
	case EMetaHumanCharacterEyelashesType::ShortSparse: return "Sparse";
	case EMetaHumanCharacterEyelashesType::ShortFine: return "ShortFine";
	case EMetaHumanCharacterEyelashesType::ShortThin: return "Thin";
	case EMetaHumanCharacterEyelashesType::LongSlightCurl: return "SlightCurl";
	case EMetaHumanCharacterEyelashesType::LongCurl: return "LongCurl";
	case EMetaHumanCharacterEyelashesType::LongThickCurl: return "ThickCurl";
#else
...
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION >= 8
	if (S.Equals(TEXT("Sparse"), ESearchCase::IgnoreCase) || S.Equals(TEXT("ShortSparse"), ESearchCase::IgnoreCase)) return EMetaHumanCharacterEyelashesType::ShortSparse;
	if (S.Equals(TEXT("ShortFine"), ESearchCase::IgnoreCase)) return EMetaHumanCharacterEyelashesType::ShortFine;
	if (S.Equals(TEXT("Thin"), ESearchCase::IgnoreCase) || S.Equals(TEXT("ShortThin"), ESearchCase::IgnoreCase)) return EMetaHumanCharacterEyelashesType::ShortThin;
	if (S.Equals(TEXT("SlightCurl"), ESearchCase::IgnoreCase) || S.Equals(TEXT("LongSlightCurl"), ESearchCase::IgnoreCase)) return EMetaHumanCharacterEyelashesType::LongSlightCurl;
	if (S.Equals(TEXT("LongCurl"), ESearchCase::IgnoreCase)) return EMetaHumanCharacterEyelashesType::LongCurl;
	if (S.Equals(TEXT("ThickCurl"), ESearchCase::IgnoreCase) || S.Equals(TEXT("LongThickCurl"), ESearchCase::IgnoreCase)) return EMetaHumanCharacterEyelashesType::LongThickCurl;
#else
...
static TArray<FLuaFunctionDoc> MetaHumanDocs = {
	...
	{ TEXT("metahuman_set_assembly(asset, params)"), TEXT("Set assembly settings: pipeline_type (Cinematic/Optimized/UEFN/DCC), quality (Low/Medium/High/Cinematic), root_directory, common_directory, animation_system_name, name_override, archive_name, output_folder, bake_makeup, export_zip"), TEXT("bool") },
	...
	{ TEXT("metahuman_parametric_fit_body(asset, target?)"), TEXT("Fit the parametric body state to a fixed body. target: 'dna' (default, ParametricFitToDnaBody) or 'compatibility' (ParametricFitToCompatibilityBody)."), TEXT("bool") },
	{ TEXT("metahuman_reset_body(asset)"), TEXT("Reset the parametric body to defaults (engine ResetParametricBody)"), TEXT("bool") },
	{ TEXT("metahuman_commit_body(asset)"), TEXT("Commit body state changes to the character asset for saving"), TEXT("bool") },
	{ TEXT("metahuman_build(asset, params?)"), TEXT("Build/assemble the MetaHuman character. params: pipeline_type, quality, root_directory, common_directory, name_override, animation_system_name, bake_makeup, export_zip, enable_wardrobe_item_validation, pipeline_override (class path string for a UMetaHumanCollectionPipeline subclass — overrides default pipeline lookup)"), TEXT("bool") },
	// Advanced body/face
	{ TEXT("metahuman_is_editing(asset)"), TEXT("Check if character is registered for editing with the subsystem"), TEXT("bool") },
	{ TEXT("metahuman_close_editing(asset)"), TEXT("Close the editing session for the character (calls RemoveObjectToEdit). Side effect: destroys any preview actor previously spawned via metahuman_spawn_actor on this character. metahuman_remove_textures_and_rigs auto-closes too."), TEXT("bool") },
	{ TEXT("metahuman_auto_rig(asset, opts?)"), TEXT("Request auto-rigging for the character. opts: rig_type ('JointsOnly'|'JointsAndBlendShapes'), blocking (default true), report_progress (default true). Returns false if a request is already active — poll with metahuman_is_auto_rigging."), TEXT("bool") },
	{ TEXT("metahuman_is_auto_rigging(asset)"), TEXT("True if a non-blocking auto-rig request is still in flight on this character"), TEXT("bool") },
	...
	{ TEXT("metahuman_request_textures(asset, opts?)"), TEXT("Request high-resolution texture synthesis. opts: blocking (default true), report_progress (default true). Returns false if a request is already active — poll with metahuman_is_requesting_textures."), TEXT("bool") },
	{ TEXT("metahuman_is_requesting_textures(asset)"), TEXT("True if a non-blocking high-resolution texture request is still in flight on this character"), TEXT("bool") },
	{ TEXT("metahuman_get_max_hf_index()"), TEXT("Maximum value the model accepts for hf_index in metahuman_estimate_skin_tone. Returns 0 + [WARN] if Face Texture Synthesizer not yet initialized — call any editing wrapper first (e.g. metahuman_get_face_gizmos(asset)) to trigger lazy init. metahuman_info does NOT register and will not initialize the synthesizer."), TEXT("int") },
	{ TEXT("metahuman_is_texture_synthesis_enabled()"), TEXT("Return whether MetaHuman face texture synthesis is initialized and skin-tone helpers can run."), TEXT("bool") },
	...
	{ TEXT("metahuman_select_face_vertex(asset, ray)"), TEXT("Hit-test a ray against the live face mesh. ray: {origin={x,y,z}, direction={x,y,z}}. Returns {vertex_index, hit={x,y,z}, normal={x,y,z}} or nil."), TEXT("table or nil") },
	{ TEXT("metahuman_add_face_landmark(asset, vertex_index)"), TEXT("Add a custom landmark manipulator at the given mesh vertex (use the vertex_index returned by metahuman_select_face_vertex)."), TEXT("bool") },
	{ TEXT("metahuman_remove_face_landmark(asset, landmark_index)"), TEXT("Remove a previously-added custom landmark by its landmark index (0-based, matches the 'index' field returned by metahuman_get_face_landmarks)."), TEXT("bool") },
	...
	// Clothing visibility
{ TEXT("metahuman_set_clothing_visibility(asset, state)"), TEXT("Set clothing visibility through the editor subsystem: 'Shown' or 'Hidden'. 'UseOverrideMaterial' requires an override material on the actor interface and is rejected by this wrapper. This wrapper supports only 'Shown' and 'Hidden' through the editor subsystem."), TEXT("bool") },
	{ TEXT("metahuman_set_preview_clothing_visibility(asset, state, material_path?)"), TEXT("Preview-only clothing visibility on spawned MetaHuman editor actors. Supports 'Shown', 'Hidden', and 'UseOverrideMaterial'; material_path overrides the subsystem translucent clothing material."), TEXT("bool") },
	{ TEXT("metahuman_get_clothing_visibility(asset)"), TEXT("Get subsystem clothing visibility state. The subsystem getter round-trips 'Shown' or 'Hidden'."), TEXT("string") },
...
#if NSAI_HAS_METAHUMAN_IDENTITY
...
#if NSAI_HAS_METAHUMAN_WARDROBE
...
static void BindMetaHuman(sol::state& Lua, FLuaSessionData& Session)
{
	...
	Lua.set_function("metahuman_info", [&Session](sol::object AssetObj, sol::this_state S) -> sol::object
	{
		...
		{
			FString Path = NeoLuaStr::ToFString(AssetObj.as<std::string>());
			MHC = NeoLuaAsset::ResolveWithRegistry<UMetaHumanCharacter>(Path);
		}
		else if (AssetObj.is<sol::table>())
		{
			...
			{
				FString Path = NeoLuaStr::ToFStringOpt(PathOpt);
				MHC = NeoLuaAsset::ResolveWithRegistry<UMetaHumanCharacter>(Path);
			}
		}
		else
...
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION >= 8
			VPT["use_custom_environment"] = VP.CharacterEnvironment == EMetaHumanCharacterEnvironment::Custom;
			VPT["custom_environment"] = TCHAR_TO_UTF8(*VP.CustomLightingEnvironment.ToSoftObjectPath().ToString());
			VPT["custom_environment_key"] = TCHAR_TO_UTF8(*VP.CustomLightingEnvironmentKey);
#else
		...
		// Assembly
		{
			...
			AST["animation_system_name"] = TCHAR_TO_UTF8(*AS.AnimationSystemName.ToString());
			AST["name_override"] = TCHAR_TO_UTF8(*AS.NameOverride);
			AST["archive_name"] = TCHAR_TO_UTF8(*AS.ArchiveName);
			AST["output_folder"] = TCHAR_TO_UTF8(*AS.OutputFolder.Path);
...
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION >= 7
		UMetaHumanCharacterEditorSubsystem* Subsystem = NeoLuaSubsystem::GetEditor<UMetaHumanCharacterEditorSubsystem>();
	...
	Lua.set_function("metahuman_set_eyes", [&Session](const std::string& AssetPath, sol::table Params, sol::this_state S) -> bool
	{
		...
		auto ApplyIris = [](FMetaHumanCharacterEyeIrisProperties& Iris, const sol::table& T)
		{
			...
			if (T["blend_method"].valid())
			{
				...
				{
					FString Method = NeoLuaStr::ToFStringOpt(MethodOpt);
	...
	Lua.set_function("metahuman_set_viewport", [&Session](const std::string& AssetPath, sol::table Params, sol::this_state S) -> bool
	{
...
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION >= 8
		if (Params["use_custom_environment"].valid())
		{
			VP.CharacterEnvironment = Params.get_or("use_custom_environment", false)
				? EMetaHumanCharacterEnvironment::Custom
				: static_cast<EMetaHumanCharacterEnvironment>(0);
		}
		if (Params["custom_environment"].valid())
		{
			sol::optional<std::string> EnvOpt = Params["custom_environment"];
			if (EnvOpt.has_value()) VP.CustomLightingEnvironment = TSoftObjectPtr<UWorld>(FSoftObjectPath(NeoLuaStr::ToFStringOpt(EnvOpt)));
		}
		if (Params["custom_environment_key"].valid())
		{
			sol::optional<std::string> EnvKeyOpt = Params["custom_environment_key"];
			if (EnvKeyOpt.has_value()) VP.CustomLightingEnvironmentKey = NeoLuaStr::ToFStringOpt(EnvKeyOpt);
		}
#else
		...
		{
			sol::optional<std::string> EnvOpt = Params["custom_environment"];
			if (EnvOpt.has_value()) VP.CustomEnvironment = FSoftObjectPath(NeoLuaStr::ToFStringOpt(EnvOpt));
		}
#endif
#endif
	...
	Lua.set_function("metahuman_set_assembly", [&Session](const std::string& AssetPath, sol::table Params, sol::this_state S) -> bool
	{
		...
		if (Params["quality"].valid())
		{
			...
			{
				FString QualStr = NeoLuaStr::ToFStringOpt(QualOpt);
		...
		if (Params["root_directory"].valid())
		{
			...
			if (DirOpt.has_value()) AS.RootDirectory.Path = NeoLuaStr::ToFStringOpt(DirOpt);
		}
		if (Params["common_directory"].valid())
		{
			sol::optional<std::string> DirOpt = Params["common_directory"];
			if (DirOpt.has_value()) AS.CommonDirectory.Path = NeoLuaStr::ToFStringOpt(DirOpt);
		}
		if (Params["animation_system_name"].valid())
		{
			sol::optional<std::string> NameOpt = Params["animation_system_name"];
			if (NameOpt.has_value()) AS.AnimationSystemName = FName(*NeoLuaStr::ToFStringOpt(NameOpt));
		}
		if (Params["name_override"].valid())
		{
			sol::optional<std::string> NameOpt = Params["name_override"];
			if (NameOpt.has_value()) AS.NameOverride = NeoLuaStr::ToFStringOpt(NameOpt);
		}
		if (Params["archive_name"].valid())
		{
			sol::optional<std::string> NameOpt = Params["archive_name"];
			if (NameOpt.has_value()) AS.ArchiveName = NeoLuaStr::ToFStringOpt(NameOpt);
		}
		if (Params["output_folder"].valid())
		{
			sol::optional<std::string> DirOpt = Params["output_folder"];
			if (DirOpt.has_value()) AS.OutputFolder.Path = NeoLuaStr::ToFStringOpt(DirOpt);
		}
		...
		sol::this_state S) -> bool
	{
		...
		FString FMeshPath = NeoLuaStr::ToFString(BodyMeshPath);
		UObject* BodyMeshObj = NeoLuaAsset::ResolveWithRegistry<UObject>(FMeshPath);
		if (!BodyMeshObj)
		{
...
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION >= 8
		bool bConformed = Subsystem->ConformBodyToTarget(MHC, Vertices, JointRotations, bRepose, bEstimateJoints);
#else
		...
		if (!bConformed)
		{
			Session.Log(FString::Printf(
				TEXT("[FAIL] metahuman_conform_body -> ConformBody returned false for %d vertices and %d joint rotations from '%s'. ")
				TEXT("Verify the body mesh is a MetaHuman-compatible SkeletalMesh; try toggling repose=false or estimate_joints=false."),
				Vertices.Num(), JointRotations.Num(), *BodyMeshObj->GetName()));
			return false;
		}
	...
	// ================================================================
	// metahuman_parametric_fit_body(asset, target?)
	//
	// target: 'dna' (default) -> ParametricFitToDnaBody
	//         'compatibility' -> ParametricFitToCompatibilityBody
	// Both engine methods are plain C++; reflection invoke can't reach them.
	// ================================================================
	Lua.set_function("metahuman_parametric_fit_body", [&Session](const std::string& AssetPath,
		sol::optional<std::string> TargetOpt, sol::this_state S) -> bool
	{
		UMetaHumanCharacter* MHC = LoadMHCharacter(AssetPath, Session);
		if (!MHC) return false;
		...
		if (!Subsystem)
		{
			Session.Log(TEXT("[FAIL] metahuman_parametric_fit_body -> subsystem not available"));
			return false;
		}
		...
		const FString Target = TargetOpt.has_value()
			? NeoLuaStr::ToFStringOpt(TargetOpt)
			: FString(TEXT("dna"));

		FScopedTransaction Tx(FText::FromString(TEXT("Parametric Fit MetaHuman Body")));
		MHC->Modify();

		bool bSuccess = false;
		if (Target.Equals(TEXT("compatibility"), ESearchCase::IgnoreCase))
		{
			bSuccess = Subsystem->ParametricFitToCompatibilityBody(MHC);
		}
		else if (Target.Equals(TEXT("dna"), ESearchCase::IgnoreCase))
		{
			bSuccess = Subsystem->ParametricFitToDnaBody(MHC);
		}
		else
		{
			Session.Log(FString::Printf(TEXT("[FAIL] metahuman_parametric_fit_body -> unknown target '%s' (expected 'dna' or 'compatibility')"),
				*Target));
			return false;
		}
		...
		{
			MHC->MarkPackageDirty();
			Session.Log(FString::Printf(TEXT("[OK] metahuman_parametric_fit_body -> %s fit succeeded"), *Target));
		}
		else
		{
			Session.Log(FString::Printf(
				TEXT("[FAIL] metahuman_parametric_fit_body -> %s fit returned false. ")
				TEXT("Character may lack the corresponding fixed body DNA; import it first."),
				*Target));
		}
		return bSuccess;
	});
	...
	// ================================================================
	// metahuman_reset_body(asset)
	// ================================================================
	Lua.set_function("metahuman_reset_body", [&Session](const std::string& AssetPath,
		sol::this_state S) -> bool
	{
		UMetaHumanCharacter* MHC = LoadMHCharacter(AssetPath, Session);
		if (!MHC) return false;
		...
		if (!Subsystem)
		{
			Session.Log(TEXT("[FAIL] metahuman_reset_body -> subsystem not available"));
			return false;
		}
		...
		FScopedTransaction Tx(FText::FromString(TEXT("Reset MetaHuman Body")));
		MHC->Modify();

		Subsystem->ResetParametricBody(MHC);

		MHC->MarkPackageDirty();
		Session.Log(TEXT("[OK] metahuman_reset_body -> reset to default parametric body"));
		return true;
	});
		...
		sol::table ConstraintsTable, sol::this_state S) -> bool
	{
		...
		for (const auto& Pair : ConstraintsTable)
		{
			...
			FName TargetName = FName(NeoLuaStr::ToFStringOpt(NameOpt));
		...
		sol::optional<sol::table> ParamsOpt, sol::this_state S) -> bool
	{
		...
		FMetaHumanCharacterEditorBuildParameters BuildParams = MakeMHBuildParamsFromAssemblySettings(MHC);
		...
		if (ParamsOpt.has_value())
		{
			...
			if (P["root_directory"].valid())
			{
				...
				if (DirOpt.has_value()) BuildParams.AbsoluteBuildPath = NeoLuaStr::ToFStringOpt(DirOpt);
			}
			if (P["common_directory"].valid())
			{
				sol::optional<std::string> DirOpt = P["common_directory"];
				if (DirOpt.has_value()) BuildParams.CommonFolderPath = NeoLuaStr::ToFStringOpt(DirOpt);
			}
			if (P["name_override"].valid())
			{
				sol::optional<std::string> NameOpt = P["name_override"];
				if (NameOpt.has_value()) BuildParams.NameOverride = NeoLuaStr::ToFStringOpt(NameOpt);
			}
			if (P["animation_system_name"].valid())
			{
				sol::optional<std::string> NameOpt = P["animation_system_name"];
				if (NameOpt.has_value()) BuildParams.AnimationSystemName = FName(*NeoLuaStr::ToFStringOpt(NameOpt));
			}
#if ENGINE_MAJOR_VERSION == 5 && ENGINE_MINOR_VERSION < 8
			if (P["bake_makeup"].valid()) BuildParams.bBakeMakeup = P.get_or("bake_makeup", true);
			if (P["export_zip"].valid()) BuildParams.bExportZipFile = P.get_or("export_zip", false);
#endif
			if (P["enable_wardrobe_item_validation"].valid())
				BuildParams.bEnableWardrobeItemValidation = P.get_or("enable_wardrobe_item_validation", true);

			// Pipeline override — accept a class path string. Mirrors the editor's
			// GetSelectedPipeline() pattern at MetaHumanCharacterEditorPipelineTools.cpp:215:
			// look up the cached instance on Character->PipelinesPerClass, NewObject if missing.
			// PipelineOverride is UPROPERTY() with no BlueprintReadWrite, so this is the only
			// way for an agent to set it (reflection invoke can't reach it).
			if (P["pipeline_override"].valid())
			{
				sol::optional<std::string> ClassPathOpt = P["pipeline_override"];
				if (ClassPathOpt.has_value())
				{
					FString ClassPath = NeoLuaStr::ToFStringOpt(ClassPathOpt);
					UClass* PipelineClass = LoadClass<UMetaHumanCollectionPipeline>(nullptr, *ClassPath);
					if (!PipelineClass)
					{
						Session.Log(FString::Printf(
							TEXT("[FAIL] metahuman_build -> pipeline_override class not found: '%s'. Use a full class path like '/Script/MyModule.MyPipelineClass' or '/Game/MyPipeline_BP.MyPipeline_BP_C'."),
							*ClassPath));
						return false;
					}
					TObjectPtr<UMetaHumanCollectionPipeline>& Cached = MHC->PipelinesPerClass.FindOrAdd(PipelineClass);
					if (Cached == nullptr)
					{
						Cached = NewObject<UMetaHumanCollectionPipeline>(MHC, PipelineClass);
					}
					BuildParams.PipelineOverride = Cached;
				}
			}
		}
	...
	// ================================================================
	// metahuman_close_editing(asset)
	//
	// Pairs with the implicit TryAddObjectToEdit performed by every other wrapper via
	// EnsureMHCharacterRegistered. Without this, a single call (e.g. metahuman_get_face_gizmos)
	// leaves CharacterDataMap state alive for the rest of the editor session, blocking
	// RemoveTexturesAndRigs and leaking preview actor state per the engine contract at
	// MetaHumanCharacterEditorSubsystem.h:475.
	// ================================================================
	Lua.set_function("metahuman_close_editing", [&Session](const std::string& AssetPath,
		sol::this_state S) -> bool
	{
		UMetaHumanCharacter* MHC = LoadMHCharacter(AssetPath, Session);
		if (!MHC) return false;
		...
		if (!Subsystem)
		{
			Session.Log(TEXT("[FAIL] metahuman_close_editing -> subsystem not available"));
			return false;
		}

		if (!Subsystem->IsObjectAddedForEditing(MHC))
		{
			Session.Log(FString::Printf(TEXT("[OK] metahuman_close_editing -> '%s' was not open for editing"),
				*MHC->GetName()));
			return true;
		}

		Subsystem->RemoveObjectToEdit(MHC);
		Session.Log(FString::Printf(TEXT("[OK] metahuman_close_editing -> closed '%s'"), *MHC->GetName()));
		return true;
	});
		...
		sol::optional<sol::table> OptsOpt, sol::this_state S) -> bool
	{
		...
		// Engine RequestAutoRigging silently no-ops (with an error log) if a request is
		// already active. Surface the rejection to the caller so non-blocking workflows
		// can poll metahuman_is_auto_rigging instead of falsely treating a no-op as queued.
		if (Subsystem->IsAutoRiggingFace(MHC))
		{
			Session.Log(TEXT("[FAIL] metahuman_auto_rig -> request already active. ")
				TEXT("Poll metahuman_is_auto_rigging(asset) until it returns false before retrying."));
			return false;
		}
		...
		if (OptsOpt.has_value())
		{
			...
			if (Opts["rig_type"].valid())
			{
				...
				{
					FString RigStr = NeoLuaStr::ToFStringOpt(RigTypeOpt);
	...
	// ================================================================
	// metahuman_is_auto_rigging(asset)
	//
	// Polling counterpart to metahuman_auto_rig(blocking=false). Engine method is plain
	// C++ (not BlueprintCallable), so reflection invoke can't reach it.
	// ================================================================
	Lua.set_function("metahuman_is_auto_rigging", [&Session](const std::string& AssetPath,
		sol::this_state S) -> bool
	{
		UMetaHumanCharacter* MHC = LoadMHCharacter(AssetPath, Session);
		if (!MHC) return false;
		...
		if (!Subsystem)
		{
			Session.Log(TEXT("[FAIL] metahuman_is_auto_rigging -> subsystem not available"));
			return false;
		}

		return Subsystem->IsAutoRiggingFace(MHC);
	});
		...
		sol::this_state S) -> sol::object
	{
		...
		FString FDNAPath = NeoLuaStr::ToFString(DNAPath);
		...
		if (OptsOpt.has_value())
		{
			...
			if (Opts["alignment"].valid())
			{
				...
				{
					FString AlignStr = NeoLuaStr::ToFStringOpt(AlignOpt);
		...
		const FString ResultStr = NeoLuaEnum::ToString<EImportErrorCode>(Result);
		...
		{
			Session.Log(FString::Printf(
				TEXT("[FAIL] metahuman_import_face_dna(%s) -> %s. ")
				TEXT("If CombinedBodyCannotBeImportedAsWholeRig, retry with whole_rig=false. ")
				TEXT("If InvalidInputData/InvalidHeadMesh, verify the DNA file matches the MetaHuman face topology."),
				*FPaths::GetCleanFilename(FDNAPath), *ResultStr));
		}

		return sol::make_object(LuaView, std::string(TCHAR_TO_UTF8(*ResultStr)));
	});
		...
		sol::optional<sol::table> OptsOpt, sol::this_state S) -> bool
	{
		...
		// Engine RequestTextureSources silently no-ops (with an error log) if a request
		// is already active. Surface the rejection so non-blocking workflows can poll
		// metahuman_is_requesting_textures instead of falsely treating a no-op as queued.
		if (Subsystem->IsRequestingHighResolutionTextures(MHC))
		{
			Session.Log(TEXT("[FAIL] metahuman_request_textures -> texture request already active. ")
				TEXT("Poll metahuman_is_requesting_textures(asset) until it returns false before retrying."));
			return false;
		}
	...
	// ================================================================
	// metahuman_is_requesting_textures(asset)
	//
	// Polling counterpart to metahuman_request_textures(blocking=false). Engine method
	// is plain C++ (not BlueprintCallable), so reflection invoke can't reach it.
	// ================================================================
	Lua.set_function("metahuman_is_requesting_textures", [&Session](const std::string& AssetPath,
		sol::this_state S) -> bool
	{
		UMetaHumanCharacter* MHC = LoadMHCharacter(AssetPath, Session);
		if (!MHC) return false;
		...
		if (!Subsystem)
		{
			Session.Log(TEXT("[FAIL] metahuman_is_requesting_textures -> subsystem not available"));
			return false;
		}

		return Subsystem->IsRequestingHighResolutionTextures(MHC);
	});

	// ================================================================
	// metahuman_get_max_hf_index()
	//
	// Returns the maximum hf_index the texture synthesis model accepts. Pairs with
	// metahuman_estimate_skin_tone(color, hf_index) so callers can clamp inputs.
	// ================================================================
	Lua.set_function("metahuman_get_max_hf_index", [&Session](sol::this_state S) -> int
	{
		UMetaHumanCharacterEditorSubsystem* Subsystem = NeoLuaSubsystem::GetEditor<UMetaHumanCharacterEditorSubsystem>();
		if (!Subsystem)
		{
			Session.Log(TEXT("[FAIL] metahuman_get_max_hf_index -> subsystem not available"));
			return 0;
		}

		// FaceTextureSynthesizer is loaded lazily on the first TryAddObjectToEdit.
		// Until then GetMaxHighFrequencyIndex returns 0 with an engine error log.
		// Surface that ambiguity so callers don't treat 0 as a real maximum.
		// NOTE: metahuman_info does NOT register the character (it only resolves+reads it),
		// so it will not trigger lazy init. Use metahuman_get_face_gizmos or any other
		// editing wrapper that calls EnsureMHCharacterRegistered.
		if (!Subsystem->IsTextureSynthesisEnabled())
		{
			Session.Log(TEXT("[WARN] metahuman_get_max_hf_index -> Face Texture Synthesizer not yet initialized. ")
				TEXT("Call any editing wrapper first (e.g. metahuman_get_face_gizmos(asset)) to trigger lazy init."));
			return 0;
		}

		const int32 Max = Subsystem->GetMaxHighFrequencyIndex();
		Session.Log(FString::Printf(TEXT("[OK] metahuman_get_max_hf_index -> %d"), Max));
		return Max;
	});

	Lua.set_function("metahuman_is_texture_synthesis_enabled", [&Session]() -> bool
	{
		UMetaHumanCharacterEditorSubsystem* Subsystem = NeoLuaSubsystem::GetEditor<UMetaHumanCharacterEditorSubsystem>();
		if (!Subsystem)
		{
			Session.Log(TEXT("[FAIL] metahuman_is_texture_synthesis_enabled -> subsystem not available"));
			return false;
		}

		const bool bEnabled = Subsystem->IsTextureSynthesisEnabled();
		Session.Log(FString::Printf(TEXT("[OK] metahuman_is_texture_synthesis_enabled -> %s"),
	...
	// ================================================================
	// metahuman_select_face_vertex(asset, ray) -> {vertex_index, hit, normal} | nil
	// ================================================================
	Lua.set_function("metahuman_select_face_vertex", [&Session](const std::string& AssetPath,
		sol::table RayTable, sol::this_state S) -> sol::object
	{
		sol::state_view LuaView(S);
		...
		if (!Subsystem)
		{
			Session.Log(TEXT("[FAIL] metahuman_select_face_vertex -> subsystem not available"));
			return sol::lua_nil;
		}
		...
		sol::optional<sol::table> OriginOpt = RayTable.get<sol::optional<sol::table>>("origin");
		sol::optional<sol::table> DirOpt = RayTable.get<sol::optional<sol::table>>("direction");
		if (!OriginOpt.has_value() || !DirOpt.has_value())
		{
			Session.Log(TEXT("[FAIL] metahuman_select_face_vertex -> ray must have origin={x,y,z} and direction={x,y,z}"));
			return sol::lua_nil;
		}

		sol::table O = OriginOpt.value();
		sol::table D = DirOpt.value();
		FRay Ray(
			FVector(O.get_or("x", 0.0), O.get_or("y", 0.0), O.get_or("z", 0.0)),
			FVector(D.get_or("x", 0.0), D.get_or("y", 0.0), D.get_or("z", 1.0)).GetSafeNormal());

		FVector HitVertex(0, 0, 0);
		FVector HitNormal(0, 0, 0);
		const int32 VertexIndex = Subsystem->SelectFaceVertex(MHC, Ray, HitVertex, HitNormal);

		if (VertexIndex == INDEX_NONE)
		{
			Session.Log(TEXT("[FAIL] metahuman_select_face_vertex -> ray missed the face mesh"));
			return sol::lua_nil;
		}

		sol::table Result = LuaView.create_table();
		Result["vertex_index"] = VertexIndex;
		sol::table HitT = LuaView.create_table();
		HitT["x"] = HitVertex.X; HitT["y"] = HitVertex.Y; HitT["z"] = HitVertex.Z;
		Result["hit"] = HitT;
		sol::table NormT = LuaView.create_table();
		NormT["x"] = HitNormal.X; NormT["y"] = HitNormal.Y; NormT["z"] = HitNormal.Z;
		Result["normal"] = NormT;

		Session.Log(FString::Printf(TEXT("[OK] metahuman_select_face_vertex -> vertex %d"), VertexIndex));
		return Result;
	});

	// ================================================================
	// metahuman_add_face_landmark(asset, vertex_index)
	// ================================================================
	Lua.set_function("metahuman_add_face_landmark", [&Session](const std::string& AssetPath,
		int VertexIndex, sol::this_state S) -> bool
	{
		UMetaHumanCharacter* MHC = LoadMHCharacter(AssetPath, Session);
		if (!MHC) return false;
		...
		if (!Subsystem)
		{
			Session.Log(TEXT("[FAIL] metahuman_add_face_landmark -> subsystem not available"));
			return false;
		}
		...
		FScopedTransaction Tx(FText::FromString(TEXT("Add MetaHuman Face Landmark")));
		MHC->Modify();

		Subsystem->AddFaceLandmark(MHC, VertexIndex);
		MHC->MarkPackageDirty();

		Session.Log(FString::Printf(TEXT("[OK] metahuman_add_face_landmark -> added at vertex %d"), VertexIndex));
		return true;
	});

	// ================================================================
	// metahuman_remove_face_landmark(asset, landmark_index)
	// ================================================================
	Lua.set_function("metahuman_remove_face_landmark", [&Session](const std::string& AssetPath,
		int LandmarkIndex, sol::this_state S) -> bool
	{
		UMetaHumanCharacter* MHC = LoadMHCharacter(AssetPath, Session);
		if (!MHC) return false;
		...
		if (!Subsystem)
		{
			Session.Log(TEXT("[FAIL] metahuman_remove_face_landmark -> subsystem not available"));
			return false;
		}
		...
		FScopedTransaction Tx(FText::FromString(TEXT("Remove MetaHuman Face Landmark")));
		MHC->Modify();

		Subsystem->RemoveFaceLandmark(MHC, LandmarkIndex);
		MHC->MarkPackageDirty();

		Session.Log(FString::Printf(TEXT("[OK] metahuman_remove_face_landmark -> removed landmark %d"), LandmarkIndex));
		return true;
	});
		...
		sol::this_state S) -> bool
	{
		...
		{
			Session.Log(FString::Printf(
				TEXT("[FAIL] metahuman_set_body_joints -> SetBodyJoints returned false for %d translations and %d rotations. ")
				TEXT("Use joint arrays from metahuman_conform_body / GetJointsForBodyConforming, and keep counts aligned with the MetaHuman body skeleton."),
				Translations.Num(), Rotations.Num()));
		}
		return bSuccess;
	});
		...
		sol::this_state S) -> bool
	{
		...
		{
			Session.Log(FString::Printf(
				TEXT("[FAIL] metahuman_set_body_mesh -> SetBodyMesh returned false for %d vertices. ")
				TEXT("Vertex count and order must match the MetaHuman body archetype topology — use vertices from metahuman_conform_body or GetMeshForBodyConforming."),
				Vertices.Num()));
		}
		return bSuccess;
	});
		...
		sol::this_state S) -> sol::object
	{
		...
		FString FMeshPath = NeoLuaStr::ToFString(TemplateMeshPath);
		UObject* TemplateMesh = NeoLuaAsset::ResolveWithRegistry<UObject>(FMeshPath);
		if (!TemplateMesh)
		{
		...
		if (OptsOpt.has_value())
		{
			...
			{
				FString P = NeoLuaStr::ToFStringOpt(LeftEyeOpt);
				LeftEyeMesh = NeoLuaAsset::ResolveWithRegistry<UObject>(P);
			}
			sol::optional<std::string> RightEyeOpt = Opts.get<sol::optional<std::string>>("right_eye_mesh");
			if (RightEyeOpt.has_value())
			{
				FString P = NeoLuaStr::ToFStringOpt(RightEyeOpt);
				RightEyeMesh = NeoLuaAsset::ResolveWithRegistry<UObject>(P);
			}
			sol::optional<std::string> TeethOpt = Opts.get<sol::optional<std::string>>("teeth_mesh");
			if (TeethOpt.has_value())
			{
				FString P = NeoLuaStr::ToFStringOpt(TeethOpt);
				TeethMesh = NeoLuaAsset::ResolveWithRegistry<UObject>(P);
			}
		}
		...
		{
			Session.Log(FString::Printf(
				TEXT("[FAIL] metahuman_import_from_template(%s) -> %s. ")
				TEXT("Verify the head mesh matches MetaHuman topology; ")
				TEXT("InvalidLeftEyeMesh/InvalidRightEyeMesh/InvalidTeethMesh point at which optional mesh failed."),
				*FPaths::GetCleanFilename(FMeshPath), *ResultStr));
		}
		...
		sol::this_state S) -> sol::object
	{
		...
		// Engine GetSkinTone uses ensureMsgf(IsTextureSynthesisEnabled(), ...) and
		// silently returns FLinearColor::Black when the synthesizer isn't initialized.
		// Reject explicitly so callers don't treat (0,0,0) as a real skin tone.
		if (!Subsystem->IsTextureSynthesisEnabled())
		{
			Session.Log(TEXT("[FAIL] metahuman_get_skin_tone -> Face Texture Synthesizer not initialized. ")
				TEXT("Open any MetaHumanCharacter for editing first (e.g. metahuman_get_face_gizmos(asset)) to trigger lazy init."));
			return sol::lua_nil;
		}
		...
		sol::this_state S) -> sol::object
	{
		...
		// Engine ProjectSkinTone returns an empty FVector2f silently when the synthesizer
		// isn't initialized. Reject explicitly so callers don't treat (0,0) as a real UV.
		if (!Subsystem->IsTextureSynthesisEnabled())
		{
			Session.Log(TEXT("[FAIL] metahuman_estimate_skin_tone -> Face Texture Synthesizer not initialized. ")
				TEXT("Open any MetaHumanCharacter for editing first (e.g. metahuman_get_face_gizmos(asset)) to trigger lazy init."));
			return sol::lua_nil;
		}
		...
		const std::string& StateStr, sol::this_state S) -> bool
	{
		...
		FString S2 = NeoLuaStr::ToFString(StateStr);
		EMetaHumanClothingVisibilityState State;
		FString ParseError;
		if (!ParseMHClothingVisibilityState(S2, State, ParseError))
		{
			Session.Log(FString::Printf(TEXT("[FAIL] metahuman_set_clothing_visibility -> %s"), *ParseError));
			return false;
		}
		if (State == EMetaHumanClothingVisibilityState::UseOverrideMaterial)
		{
			// The actor interface supports UseOverrideMaterial only when an override
			// material is provided. The subsystem API used here has no material
			// parameter and its getter round-trips only Shown/Hidden.
			Session.Log(FString::Printf(
				TEXT("[FAIL] metahuman_set_clothing_visibility -> '%s' is preview-only. ")
				TEXT("Use metahuman_set_preview_clothing_visibility(asset, 'UseOverrideMaterial', material_path?) to route an override material to the editor actor interface."),
				*S2));
			return false;
		}
	...
	// ================================================================
	// metahuman_set_preview_clothing_visibility(asset, state, material_path?)
	// ================================================================
	Lua.set_function("metahuman_set_preview_clothing_visibility", [&Session](const std::string& AssetPath,
		const std::string& StateStr, sol::optional<std::string> MaterialPathOpt, sol::this_state S) -> bool
	{
		UMetaHumanCharacter* MHC = LoadMHCharacter(AssetPath, Session);
		if (!MHC) return false;
		...
		if (!Subsystem)
		{
			Session.Log(TEXT("[FAIL] metahuman_set_preview_clothing_visibility -> subsystem not available"));
			return false;
		}
		...
		if (!ParseMHClothingVisibilityState(S2, State, ParseError))
		{
			Session.Log(FString::Printf(TEXT("[FAIL] metahuman_set_preview_clothing_visibility -> %s"), *ParseError));
			return false;
		}
		...
		if (State == EMetaHumanClothingVisibilityState::UseOverrideMaterial)
		{
			if (MaterialPathOpt.has_value() && !MaterialPathOpt.value().empty())
			{
				const FString MaterialPath = NeoLuaStr::ToFStringOpt(MaterialPathOpt);
				OverrideMaterial = NeoLuaAsset::Resolve<UMaterialInterface>(MaterialPath);
				if (!OverrideMaterial)
				{
					Session.Log(FString::Printf(TEXT("[FAIL] metahuman_set_preview_clothing_visibility -> material not found: %s"), *MaterialPath));
					return false;
				}
			}
			else
			{
				OverrideMaterial = Subsystem->GetTranslucentClothingMaterial();
				if (!OverrideMaterial)
				{
					Session.Log(TEXT("[FAIL] metahuman_set_preview_clothing_visibility -> subsystem translucent clothing material is unavailable; pass material_path"));
					return false;
				}
			}
		}
		...
		Subsystem->ForEachCharacterActor(MHC, [&AppliedCount, State, OverrideMaterial](TScriptInterface<IMetaHumanCharacterEditorActorInterface> Actor)
		{
			Actor->SetClothingVisibilityState(State, OverrideMaterial);
			++AppliedCount;
		});

		if (AppliedCount <= 0)
		{
			Session.Log(TEXT("[FAIL] metahuman_set_preview_clothing_visibility -> no spawned editor preview actor found; call metahuman_spawn_actor(asset) first"));
			return false;
		}

		Session.Log(FString::Printf(TEXT("[OK] metahuman_set_preview_clothing_visibility -> %s on %d preview actor(s)"), *S2, AppliedCount));
		return true;
	});
		...
		const std::string& TypeStr, sol::this_state S) -> bool
	{
		...
		FString TS = NeoLuaStr::ToFString(TypeStr);
		EMetaHumanCharacterSkinPreviewMaterial MatType;
		if (TS.Equals(TEXT("Topology"), ESearchCase::IgnoreCase))
		{
			MatType = EMetaHumanCharacterSkinPreviewMaterial::Default;
		}
		else if (TS.Equals(TEXT("Skin"), ESearchCase::IgnoreCase))
		{
		...
		{
			Session.Log(FString::Printf(
				TEXT("[FAIL] metahuman_update_preview_material -> unknown preview type '%s'. Expected 'Topology', 'Skin', or 'Clay'."),
				*TS));
			return false;
		}
		...
		sol::this_state S) -> bool
	{
		...
		// Engine RemoveTexturesAndRigs requires the character NOT be open for editing.
		// Auto-close any prior implicit edit session opened by other metahuman_* wrappers,
		// so a single-call sequence (e.g. metahuman_get_face_gizmos -> metahuman_remove_textures_and_rigs)
		// works without the caller needing to invoke metahuman_close_editing manually.
		if (Subsystem->IsObjectAddedForEditing(MHC))
		{
			Subsystem->RemoveObjectToEdit(MHC);
			Session.Log(FString::Printf(TEXT("[INFO] metahuman_remove_textures_and_rigs -> auto-closed editing session for '%s'"),
				*MHC->GetName()));
		}
		...
		{
			Session.Log(TEXT("[FAIL] metahuman_remove_textures_and_rigs -> engine RemoveTexturesAndRigs returned false. ")
				TEXT("This usually means the character has no removable rigs/textures, or another tool has it open."));
		}
		return bSuccess;
	});
		...
		sol::this_state S) -> sol::object
	{
		...
		if (!SpawnedActor)
		{
			Session.Log(TEXT("[FAIL] metahuman_spawn_actor -> SpawnMetaHumanActor returned null. ")
				TEXT("Ensure an editor world is open (GWorld is the level editor) and that the character has a valid pipeline / preview assets."));
			return sol::lua_nil;
		}
		...
		sol::table Opts, sol::this_state S) -> sol::object
	{
		...
		{
			FString BO = NeoLuaStr::ToFStringOpt(BlendOptStr);
		...
		// Engine BlendFaceRegion (cpp:2594) only mutates when PresetStates.Num() <= Weights.Num().
		// Fewer weights than presets is a silent no-op in the engine — reject explicitly so
		// callers don't see a [OK] log while nothing actually blended.
		if (PresetStates.Num() > Weights.Num())
		{
			Session.Log(FString::Printf(
				TEXT("[FAIL] metahuman_blend_face_region -> %d preset_assets requires at least %d weights (got %d). Engine no-ops when presets > weights."),
				PresetStates.Num(), PresetStates.Num(), Weights.Num()));
			return sol::lua_nil;
		}
		...
		sol::table Opts, sol::this_state S) -> sol::object
	{
		...
		// Engine BlendBodyRegion (cpp:4976) only mutates when PresetStates.Num() <= Weights.Num().
		// Fewer weights than presets is a silent no-op — reject explicitly.
		if (PresetStates.Num() > Weights.Num())
		{
			Session.Log(FString::Printf(
				TEXT("[FAIL] metahuman_blend_body_region -> %d preset_assets requires at least %d weights (got %d). Engine no-ops when presets > weights."),
				PresetStates.Num(), PresetStates.Num(), Weights.Num()));
			return sol::lua_nil;
		}
		...
		sol::table Opts, sol::this_state S) -> bool
	{
		...
		{
			Session.Log(FString::Printf(
				TEXT("[FAIL] metahuman_fit_face_to_vertices -> engine FitStateToTargetVertices returned false. ")
				TEXT("Check head_vertices topology matches MetaHuman head archetype (%d head verts supplied); ")
				TEXT("if optional eye/teeth meshes were supplied, they must match the corresponding archetype too. ")
				TEXT("Try alignment='ScalingRotationTranslation' (default) or set disable_hf_delta=true."),
				Params.HeadVertices.Num()));
		}
		return bSuccess;
	});
		...
		sol::this_state S) -> sol::object
	{
		...
		FString FIdPath = NeoLuaStr::ToFString(IdentityPath);
		UObject* IdentityObj = NeoLuaAsset::ResolveWithRegistry<UObject>(FIdPath);
		...
		{
			Session.Log(FString::Printf(
				TEXT("[FAIL] metahuman_import_from_identity(%s) -> %s. ")
				TEXT("Ensure the identity contains conformed head/eye/teeth meshes matching the selected use_eyes/use_teeth options. ")
				TEXT("If IdentityNotConformed, run identity conforming first."),
				*FPaths::GetCleanFilename(FIdPath), *ResultStr));
		}

		return sol::make_object(LuaView, std::string(TCHAR_TO_UTF8(*ResultStr)));
	});
#endif // NSAI_HAS_METAHUMAN_IDENTITY
		...
		const std::string& OutputPath, sol::this_state S) -> sol::object
	{
		...
		{
			Session.Log(FString::Printf(
				TEXT("[FAIL] metahuman_create_combined_mesh -> CreateCombinedFaceAndBodyMesh returned null for output '%s'. ")
				TEXT("Character must have BOTH face DNA and body DNA — check metahuman_info(asset).has_face_dna and .has_body_dna; if missing, run metahuman_auto_rig or import DNA first."),
				*FOutputPath));
			return sol::lua_nil;
		}
		...
		sol::this_state S) -> bool
	{
		...
		FString FWIPath = NeoLuaStr::ToFString(WardrobeItemPath);
		UMetaHumanWardrobeItem* WardrobeItem = NeoLuaAsset::ResolveWithRegistry<UMetaHumanWardrobeItem>(FWIPath);
		if (!WardrobeItem)
		{
		...
		FName SlotName = FName(NeoLuaStr::ToFString(SlotNameStr));
...
#endif // NSAI_HAS_METAHUMAN_WARDROBE
}
...
namespace NSAIMetaHumanExtension
{
	const TArray<FLuaFunctionDoc>& GetMetaHumanLuaDocs()
	{
		static const TArray<FLuaFunctionDoc> EmptyDocs;
		return EmptyDocs;
	}

	void BindMetaHumanLua(sol::state& /*Lua*/, FLuaSessionData& Session)
	{
		Session.Log(TEXT("[WARN] MetaHuman extension is compiled without MetaHuman support."));
	}
}

#endif // !NSAI_METAHUMAN_DISABLED
