...
`GE_Respawn_Reset` (in the Effects folder) is the health/stamina restore: an **Instant** GE whose two `Override` modifiers set Health→MaxHealth and Stamina→MaxStamina via *attribute-based* magnitude (coefficient 1, captured from Target). This exists because a direct reset isn't reachable any other way here: the `UDeadReckoningAttributeSet` attributes are `BlueprintReadOnly` (no `Set Health` node compiles), the attribute set has **no clamping** (so an `AddBase` heal would overshoot 100 — there's no `PostGameplayEffectExecute`), and `SetNumericAttributeBase` isn't Blueprint-exposed. Author GE modifiers through the **NeoStack GameplayEffect bridge** (`bp:add("modifier", {attribute="DeadReckoningAttributeSet.Health", op="Override", magnitude={type="AttributeBased", backing_attribute="DeadReckoningAttributeSet.MaxHealth", ...}})`), **not** Python `set_editor_property` — nearly every `GameplayModifierInfo` field is `EditDefaultsOnly` and the Python setter rejects it ("cannot be edited on instances").
