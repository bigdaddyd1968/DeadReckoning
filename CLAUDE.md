...
Taking damage also spawns a floating **`WBP_DamageNumber`** popup. The `IA_Damage` path, after applying `GE_Status_Damage`, computes the damage taken (`Health Current Value` after − `CurrentHealth` before → a negative delta), `Create Widget`s the popup, calls its `SetDamage(Amount)` **function**, `Add`s it to the viewport, and positions it at the player via `Project World Location to Widget Position` → `Set Position in Viewport` (+ `Set Alignment 0.5,0.5`). `SetDamage` is a **function, not a custom event** — custom events aren't callable from another Blueprint, so passing per-instance data into a freshly created widget needs either a function or an expose-on-spawn variable. The popup animates itself on its own `Event Tick`: an `Elapsed` accumulator drives `Set Render Translation` (float up) and `Set Render Opacity` (fade), then `Remove from Parent` once `Elapsed >= 1`. UMG animations would be the usual tool but the tooling can't author their tracks (see the fade note above), so this uses Tick like the death screen.
